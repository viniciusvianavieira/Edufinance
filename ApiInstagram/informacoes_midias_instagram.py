...
ids_midias_na_base = (pd.read_sql(f'''select Numero,Id,UTC_da_postagem
                                        from informacoes_midias_instagram
                                        order by UTC_da_postagem desc
                                        limit 500;
''', con= aws.engine)) #pegando os ultimos ids do bancos de dados
...
for i, id_foto in enumerate(ids_midias_na_base['Id'].to_list()): #COMPARAR OS IDS COM OS IDS DA BASE
    if tempo_agora < (ids_midias_na_base.iloc[i, 2] + relativedelta(minutes=15)): #o tempo de agora for maior que o tempo da publicação + 15 minutos então
        print("Postou a foto ",id_foto," menos de 15 minutos, foi adicionado")
